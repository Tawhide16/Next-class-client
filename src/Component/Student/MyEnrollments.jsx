...
const MyEnrollments = () => {
    ...
    const fetchEnrollments = async () => {
      try {
        const res = await axiosSecure.get(
          `/api/my-enrollments?studentEmail=${user.email}`
        );
  ...
  const handleSubmit = async (assignmentId) => {
    ...
    try {
      ...
      await axiosSecure.patch(
        `/api/assignments/increment-submission/${assignmentId}`
      );
  ...
  const handleTERSubmit = async () => {
    ...
    try {
      ...
      if (terImage) {
        ...
        formData.append("image", terImage);
  ...
  const currentData = enrolledClasses.slice(startIndex, startIndex + itemsPerPage);
          ...
          <div className="fixed inset-0 bg-black bg-opacity-40 backdrop-blur-sm z-50 flex justify-center items-center">
            <div className="bg-white dark:bg-gray-900 rounded-lg p-6 w-full max-w-md">
              ...
              <input
                type="file"
                accept="image/*"
                onChange={(e) => setTerImage(e.target.files[0])}
                className="mb-4"
              />
  ...
  return (
    <div className="max-w-5xl mx-auto p-4">
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
        {currentData.map(({ _id, title, teacherName, image, classId }) => (
          <div
            key={_id}
